...
async def test_quiet_end_goes_idle(tracked) -> None:
    ...
    assert tracker.get(1, "main").status == "idle"
    await supervisor.stop(1, "main")
...
async def test_queued_messages_stay_working(tracked) -> None:
    ...
    assert "queued" in tracker.get(1, "main").reason
    await supervisor.stop(1, "main")
...
async def test_failed_status_outlives_deregistration(tracked) -> None:
    ...
    async with asyncio.timeout(5):
        while supervisor.get(1, "main") is not None:
...
async def test_cleanup_discards_entry(tracked) -> None:
    ...
    tracker.discard(1)
    assert tracker.get(1, "main") is None
...
async def test_session_recovered_lands_idle_from_starting(tracked) -> None:
    ...
    tracker.session_recovered(1, "main")
...
async def test_recovery_failed_lands_failed_from_starting(tracked) -> None:
    ...
    tracker.recovery_failed(1, "main", "resume failed: agent exited before ready")
...
def test_recovery_failed_is_terminal(tracked) -> None:
    ...
    tracker.recovering(1, "main")
    tracker.recovery_failed(1, "main", "boom")

    tracker.session_recovered(1, "main")

    assert tracker.get(1, "main").status == "failed"
    assert tracker.get(1, "main").reason == "boom"
...
async def test_ask_enters_waiting_input_and_posts_question(tracked) -> None:
    ...
    assert transitions[-1]["from"] == "working"
    assert tracker.get(1, "main").status == "waiting-input"
    ...
    pending = tracker.pending(1, "main")
    ...
    assert snap[1]["main"]["question"]["id"] == pending.id
...
async def test_answering_ask_clears_and_returns_to_working(tracked) -> None:
    ...
    resolved = tracker.answer_pending(1, "main")
    ...
    async with asyncio.timeout(5):
        while True:
            ...
            if event.type == "question_resolved":
                assert event.payload == {"task_id": 1, "session": "main", "question_id": pending.id}
                break

    assert tracker.pending(1, "main") is None
    transitions = await wait_for_status(queue, "working")
...
async def test_silence_stalls_a_working_session(tracked_stall) -> None:
    ...
    assert tracker.get(1, "main").status == "stalled"
    await supervisor.stop(1, "main")
...
async def test_exit_during_waiting_discards_pending_no_resolve(tracked) -> None:
    ...
    await handle.prompt("ask")
    await wait_for_status(queue, "waiting-input")
    assert tracker.pending(1, "main") is not None
